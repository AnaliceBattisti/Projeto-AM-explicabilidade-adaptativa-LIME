...
if __name__ == "__main__":
    features_800 = [
        ("loan_percent_income > 0.23",      0.2301),
        ("person_income <= 39300.00",        0.1589),
        ("person_home_ownership=0",         -0.1200),
        ("loan_intent=4",                   -0.0290),
        ("10.99 < loan_int_rate <= 13.48",   0.0310),
    ]

    features_1600 = [
        ("loan_percent_income > 0.23",      0.2448),
        ("person_income <= 39300.00",        0.1704),
        ("person_home_ownership=0",         -0.1151),
        ("loan_intent=4",                   -0.0331),
        ("10.99 < loan_int_rate <= 13.48",   0.0274),
    ]

    texto_a = lime_to_text(features_800,  "Adimplente", 0.052)
    texto_b = lime_to_text(features_1600, "Adimplente", 0.052)

    print("--- TEXTO A (800 perturbações) ---")
    print(texto_a)
    print("\n--- TEXTO B (1600 perturbações) ---")
    print(texto_b)
    print("\nConsultando árbitro semântico (aguarde até 60s)...\n")

    try:
        resultado = check_convergence(texto_a, texto_b)

        print(f"converged:          {resultado['converged']}")
        print(f"reason:             {resultado['reason']}")
        print(f"top_features_match: {resultado['top_features_match']}")
        print(f"direction_match:    {resultado['direction_match']}")
        print(f"confidence:         {resultado['confidence']}")

        parar = resultado["converged"] and resultado["confidence"] >= 0.7
        print(f"\nLoop deve parar?    {parar}")
